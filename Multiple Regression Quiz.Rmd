...
The signs are different. This may be the effect of the feature ```bed_bath_rooms```, the intercept of which is quite large. Perhaps on thier own, the bedroom and bathroom features have less of an inpact on housing prices than thier product, and so the weight of the effect of the bathroom is reduced in this model. 
...
The warning message above for ```model3``` suggests that it is not full rank: this means that some of the information captured in one feature is also represented in another. It may be the case that ```model3``` is overfitting the training data, and so performs less well on the test data.
